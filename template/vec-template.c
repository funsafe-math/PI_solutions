...
#define N 100
#define TEST 0
// 1 - ulatwia testowanie programu,
// 0 - dla wersji przeznaczonej do automatycznej oceny

// Returns n evenly spaced samples, calculated over the interval [start,
// stop]. 0 <= n <= N for n = 0 return empty array for n = 1 return
// one-element array, with array[0] = start
...
int main(void) {
  ...
  case 1: // linspace
    if (TEST)
      printf("LINSPACE: Wpisz liczby: start, stop, n ");
  ...
  case 5: // range
    if (TEST)
      printf("RANGE: Wpisz liczby: start, step, n: ");
