...
void print_histogram(double T[], int n, int starting_from) {
  double max_v = T[0];
  for (int i = 1; i < n; i++) {
    max_v = T[i] > max_v ? T[i] : max_v;
  }
  char buf[100];
  int row_n_max_width = sprintf(buf, "%d", n - 1 + starting_from);
  char l_format[100];
  ...
  double bar_scale =
      (double)(term_width - left_text_len - right_text_len) / max_v;

  for (int i = 0; i < n; i++) {
    printf(l_format, i + starting_from);
    int chars = bar_scale * T[i];
